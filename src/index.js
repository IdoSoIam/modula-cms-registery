...
function getOwnerApiKey(env) {
    return env.OWNER_API_KEY?.trim() || '';
}
function getCustomApiKey(env) {
...
function getCapabilitiesFromToken(token, env) {
...
function templateAssetPublicUrl(env, request, id) {
    return `${baseUrl(env, request)}/public/template-assets/${encodeURIComponent(id)}`;
}
function templateAssetPublicUrlFromReference(env, request, asset) {
...
async function resolveTemplateAssetBySourceUrl(env, sourceUrl) {
    const normalized = (sourceUrl || '').trim();
    if (!normalized)
        return null;
    const row = await env.DB.prepare('SELECT id, source_url FROM template_assets WHERE source_url = ? ORDER BY created_at DESC LIMIT 1').bind(normalized).first();
    if (!row?.id) {
        return null;
    }
    return {
...
async function normalizeTemplatePreviewImage(env, request, previewImage, snapshot) {
...
function getRequestCapabilities(request, env) {
    return getCapabilitiesFromToken(readBearerToken(request), env);
}
function assertTemplateMutationAllowed(capabilities, sourceType) {
...
async function listTemplateVersions(env, templateId) {
    const result = await env.DB.prepare('SELECT id, version_number, status, created_at FROM template_versions WHERE template_id = ? ORDER BY version_number ASC').bind(templateId).all();
...
async function rowToTemplate(env, request, row) {
    const versions = await listTemplateVersions(env, row.id);
    const currentVersion = row.current_version_id
        ? await env.DB.prepare('SELECT snapshot_json, version_number FROM template_versions WHERE id = ?').bind(row.current_version_id).first()
        : null;
    const snapshot = currentVersion ? parseJson(currentVersion.snapshot_json, null) : null;
    return {
        id: row.id,
        slug: row.slug,
        label: parseJson(row.label_json, { fr: row.slug, en: row.slug }),
        description: parseJson(row.description_json, { fr: '', en: '' }),
        icon: row.icon,
        ...
        themeNames: parseJson(row.theme_names_json, []),
...
function parseReleaseMeta(manifest) {
    return parseJson(JSON.stringify(manifest?.registryMeta || {}), {});
}
function decorateRelease(request, env, row) {
...
async function listTemplates(request, env) {
    const result = await env.DB.prepare('SELECT * FROM templates WHERE deleted_at IS NULL ORDER BY updated_at DESC').all();
    const templates = await Promise.all(result.results.map(row => rowToTemplate(env, request, row)));
    return json(templates);
}
async function getTemplate(request, env, slug) {
    const row = await env.DB.prepare('SELECT * FROM templates WHERE slug = ? AND deleted_at IS NULL').bind(slug).first();
    if (!row)
        return json({ message: 'Template not found' }, { status: 404 });
    return json(await rowToTemplate(env, request, row));
}
async function createTemplate(request, env, capabilities) {
    ...
    const existing = await env.DB.prepare('SELECT * FROM templates WHERE slug = ?').bind(body.slug).first();
    ...
    if (existing?.id && existing.deleted_at) {
        const versionCount = await env.DB.prepare('SELECT MAX(version_number) as maxVersion FROM template_versions WHERE template_id = ?').bind(existing.id).first();
        const nextVersion = Number(versionCount?.maxVersion || 0) + 1;
        await env.DB.batch([
            env.DB.prepare(`INSERT INTO template_versions
        ...
        VALUES (?, ?, ?, 'published', ?, ?, ?, ?, ?, ?, ?, ?)`).bind(versionId, existing.id, nextVersion, JSON.stringify(body.snapshot || null), labelJson, descriptionJson, icon, previewImage, highlightsJson, themeNamesJson, now),
            env.DB.prepare("UPDATE template_versions SET status = 'archived' WHERE template_id = ? AND status = 'published' AND id != ?").bind(existing.id, versionId),
            env.DB.prepare(`UPDATE templates
         ...
         WHERE id = ?`).bind(labelJson, descriptionJson, icon, previewImage, highlightsJson, themeNamesJson, sourceType, versionId, now, existing.id)
        ]);
        const restored = await env.DB.prepare('SELECT * FROM templates WHERE id = ?').bind(existing.id).first();
        return json(await rowToTemplate(env, request, restored), { status: 201 });
    }
    try {
        await env.DB.batch([
            env.DB.prepare(`INSERT INTO templates
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?, ?)`).bind(id, body.slug, labelJson, descriptionJson, icon, previewImage, highlightsJson, themeNamesJson, sourceType, versionId, now, now),
            env.DB.prepare(`INSERT INTO template_versions
        (id, template_id, version_number, status, snapshot_json, label_json, description_json, icon, preview_image, highlights_json, theme_names_json, created_at)
        VALUES (?, ?, 1, 'published', ?, ?, ?, ?, ?, ?, ?, ?)`).bind(versionId, id, JSON.stringify(body.snapshot || null), labelJson, descriptionJson, icon, previewImage, highlightsJson, themeNamesJson, now)
        ]);
    }
    catch (error) {
...
async function createTemplateVersion(request, env, slug, capabilities) {
    const body = await readJson(request);
    const template = await env.DB.prepare('SELECT * FROM templates WHERE slug = ? AND deleted_at IS NULL').bind(slug).first();
    if (!template)
        return json({ message: 'Template not found' }, { status: 404 });
    assertTemplateMutationAllowed(capabilities, template.source_type === 'system' ? 'system' : 'custom');
    const versionCount = await env.DB.prepare('SELECT MAX(version_number) as maxVersion FROM template_versions WHERE template_id = ?').bind(template.id).first();
    const nextVersion = Number(versionCount?.maxVersion || 0) + 1;
    const versionId = newId('tplver');
    const now = nowIso();
    const labelJson = JSON.stringify(body.label || parseJson(template.label_json, { fr: template.slug, en: template.slug }));
    const descriptionJson = JSON.stringify(body.description || parseJson(template.description_json, { fr: '', en: '' }));
    ...
    await env.DB.batch([
        env.DB.prepare(`INSERT INTO template_versions
      (id, template_id, version_number, status, snapshot_json, label_json, description_json, icon, preview_image, highlights_json, theme_names_json, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(versionId, template.id, nextVersion, 'draft', JSON.stringify(body.snapshot || null), labelJson, descriptionJson, icon, previewImage, highlightsJson, themeNamesJson, now),
        env.DB.prepare('UPDATE templates SET label_json = ?, description_json = ?, icon = ?, preview_image = ?, highlights_json = ?, theme_names_json = ?, updated_at = ? WHERE id = ?').bind(labelJson, descriptionJson, icon, previewImage, highlightsJson, themeNamesJson, now, template.id)
    ]);
    const row = await env.DB.prepare('SELECT * FROM templates WHERE id = ?').bind(template.id).first();
    return json(await rowToTemplate(env, request, row));
}
async function publishTemplateVersion(request, env, slug, versionId, capabilities) {
    const template = await env.DB.prepare('SELECT * FROM templates WHERE slug = ? AND deleted_at IS NULL').bind(slug).first();
    if (!template)
        return json({ message: 'Template not found' }, { status: 404 });
    assertTemplateMutationAllowed(capabilities, template.source_type === 'system' ? 'system' : 'custom');
    const version = await env.DB.prepare('SELECT * FROM template_versions WHERE id = ? AND template_id = ?').bind(versionId, template.id).first();
    if (!version)
    ...
    await env.DB.batch([
        ...
        env.DB.prepare('UPDATE templates SET current_version_id = ?, label_json = ?, description_json = ?, icon = ?, preview_image = ?, highlights_json = ?, theme_names_json = ?, updated_at = ? WHERE id = ?').bind(versionId, version.label_json || template.label_json, version.description_json || template.description_json, version.icon || template.icon, version.preview_image ?? template.preview_image, version.highlights_json || template.highlights_json, version.theme_names_json || template.theme_names_json, now, template.id)
    ]);
    const row = await env.DB.prepare('SELECT * FROM templates WHERE id = ?').bind(template.id).first();
    return json(await rowToTemplate(env, request, row));
}
async function deleteTemplateVersion(request, env, slug, versionId, capabilities) {
    const template = await env.DB.prepare('SELECT * FROM templates WHERE slug = ? AND deleted_at IS NULL').bind(slug).first();
    if (!template)
        return json({ message: 'Template not found' }, { status: 404 });
    assertTemplateMutationAllowed(capabilities, template.source_type === 'system' ? 'system' : 'custom');
    ...
    const publishedVersion = template.current_version_id
        ? await env.DB.prepare('SELECT * FROM template_versions WHERE id = ? AND template_id = ?').bind(template.current_version_id, template.id).first()
        : null;
    const now = nowIso();
    await env.DB.batch([
        env.DB.prepare('DELETE FROM template_versions WHERE id = ? AND template_id = ?').bind(versionId, template.id),
        env.DB.prepare('UPDATE templates SET label_json = ?, description_json = ?, icon = ?, preview_image = ?, highlights_json = ?, theme_names_json = ?, updated_at = ? WHERE id = ?').bind(publishedVersion?.label_json || template.label_json, publishedVersion?.description_json || template.description_json, publishedVersion?.icon || template.icon, publishedVersion?.preview_image ?? template.preview_image, publishedVersion?.highlights_json || template.highlights_json, publishedVersion?.theme_names_json || template.theme_names_json, now, template.id)
    ]);
    const row = await env.DB.prepare('SELECT * FROM templates WHERE id = ?').bind(template.id).first();
    return json(await rowToTemplate(env, request, row));
}
async function deleteTemplate(env, slug, capabilities) {
    const template = await env.DB.prepare('SELECT source_type FROM templates WHERE slug = ? AND deleted_at IS NULL').bind(slug).first();
    if (!template)
        return json({ ok: true });
    assertTemplateMutationAllowed(capabilities, template.source_type === 'system' ? 'system' : 'custom');
...
async function createTemplateAsset(request, env) {
    const body = await readJson(request);
    ...
    const now = nowIso();
    await env.DB.prepare('INSERT INTO template_assets (id, filename, content_type, size, checksum, storage_key, source_url, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').bind(id, body.filename, body.contentType || 'application/octet-stream', bytes.byteLength, body.checksum || '', storageKey, body.sourceUrl || '', now).run();
    return json({
        id,
...
async function getTemplateAssetBySource(request, env) {
    const url = new URL(request.url);
    const sourceUrl = (url.searchParams.get('sourceUrl') || '').trim();
    if (!sourceUrl) {
        return json({ message: 'sourceUrl is required' }, { status: 400 });
    }
    const row = await env.DB.prepare('SELECT * FROM template_assets WHERE source_url = ? ORDER BY created_at DESC LIMIT 1').bind(sourceUrl).first();
    if (!row) {
...
async function downloadTemplateAsset(request, env, id) {
    const row = await env.DB.prepare('SELECT * FROM template_assets WHERE id = ?').bind(id).first();
    if (!row)
...
async function publicTemplateAsset(env, id) {
    const row = await env.DB.prepare('SELECT * FROM template_assets WHERE id = ?').bind(id).first();
    if (!row)
        return new Response('Not found', { status: 404 });
    const object = await env.ASSETS.get(row.storage_key);
...
async function introspectAuth(request, env) {
...
async function listReleases(request, env) {
    ...
    const result = await env.DB.prepare('SELECT * FROM releases ORDER BY created_at DESC LIMIT ? OFFSET ?').bind(limit, offset).all();
    const totalRow = await env.DB.prepare('SELECT COUNT(*) as total FROM releases').first();
...
async function createRelease(request, env) {
    const body = await readJson(request);
    ...
    const now = nowIso();
    const existing = await env.DB.prepare('SELECT id FROM releases WHERE version = ?').bind(body.version).first();
    if (existing?.id) {
        await env.DB.prepare('UPDATE releases SET channel = ?, checksum = ?, artifact_key = ?, manifest_json = ?, created_at = ? WHERE id = ?').bind(body.channel || 'stable', body.checksum || '', artifactKey, JSON.stringify(body.manifest || {}), now, existing.id).run();
    }
    else {
        await env.DB.prepare('INSERT INTO releases (id, version, channel, checksum, artifact_key, manifest_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)').bind(newId('rel'), body.version, body.channel || 'stable', body.checksum || '', artifactKey, JSON.stringify(body.manifest || {}), now).run();
    }
    return await listReleases(request, env);
}
async function getRelease(request, env, version) {
    const row = await env.DB.prepare('SELECT * FROM releases WHERE version = ?').bind(version).first();
    if (!row)
        return json({ message: 'Release not found' }, { status: 404 });
    return json(decorateRelease(request, env, row));
}
async function downloadReleaseArtifact(env, version) {
    const row = await env.DB.prepare('SELECT * FROM releases WHERE version = ?').bind(version).first();
    if (!row)
        return new Response('Not found', { status: 404 });
...
async function registerInstance(request, env) {
    const body = await readJson(request);
    const now = nowIso();
    const existing = await env.DB.prepare('SELECT * FROM instances WHERE slug = ?').bind(body.slug).first();
    if (existing) {
        await env.DB.prepare('UPDATE instances SET name = ?, environment = ?, release_channel = ?, last_seen_at = ?, updated_at = ? WHERE slug = ?').bind(body.name, body.environment || 'development', body.releaseChannel || 'stable', now, now, body.slug).run();
    }
    else {
        await env.DB.prepare('INSERT INTO instances (id, slug, name, environment, release_channel, current_version, last_seen_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, NULL, ?, ?, ?)').bind(newId('inst'), body.slug, body.name, body.environment || 'development', body.releaseChannel || 'stable', now, now, now).run();
    }
    const row = await env.DB.prepare('SELECT * FROM instances WHERE slug = ?').bind(body.slug).first();
    return json({
        id: row.id,
        slug: row.slug,
        name: row.name,
...
async function createDeployment(request, env) {
    const body = await readJson(request);
    const id = body.id || newId('dep');
    const now = nowIso();
    const existing = await env.DB.prepare('SELECT id FROM deployment_jobs WHERE id = ?').bind(id).first();
    if (existing?.id) {
        await env.DB.prepare('UPDATE deployment_jobs SET instance_slug = ?, version = ?, status = ?, metadata_json = ?, updated_at = ? WHERE id = ?').bind(body.instanceSlug, body.version, body.status || 'pending', JSON.stringify(body.metadata || null), now, id).run();
    }
    else {
        await env.DB.prepare('INSERT INTO deployment_jobs (id, instance_slug, version, status, metadata_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)').bind(id, body.instanceSlug, body.version, body.status || 'pending', JSON.stringify(body.metadata || null), now, now).run();
    }
    ...
    for (const log of logs) {
        await env.DB.prepare('INSERT OR REPLACE INTO deployment_logs (id, deployment_id, level, message, created_at) VALUES (?, ?, ?, ?, ?)').bind(log.id || newId('log'), id, log.level || 'info', log.message || '', log.createdAt || now).run();
    }
    return await getDeployment(env, id);
}
async function updateDeployment(request, env, id) {
    const body = await readJson(request);
    const existing = await env.DB.prepare('SELECT * FROM deployment_jobs WHERE id = ?').bind(id).first();
    if (!existing)
        return json({ message: 'Deployment not found' }, { status: 404 });
    const status = body.status || existing.status;
    const metadata = body.metadata === undefined ? parseJson(existing.metadata_json, null) : body.metadata;
    const updatedAt = nowIso();
    await env.DB.prepare('UPDATE deployment_jobs SET status = ?, metadata_json = ?, updated_at = ? WHERE id = ?').bind(status, JSON.stringify(metadata || null), updatedAt, id).run();
    if (Array.isArray(body.logs)) {
        for (const log of body.logs) {
            await env.DB.prepare('INSERT OR REPLACE INTO deployment_logs (id, deployment_id, level, message, created_at) VALUES (?, ?, ?, ?, ?)').bind(log.id || newId('log'), id, log.level || 'info', log.message || '', log.createdAt || updatedAt).run();
        }
    }
    if (status === 'completed' || status === 'rolled_back') {
        await env.DB.prepare('UPDATE instances SET current_version = ?, last_seen_at = ?, updated_at = ? WHERE slug = ?').bind(body.version || existing.version, updatedAt, updatedAt, body.instanceSlug || existing.instance_slug).run();
    }
    return await getDeployment(env, id);
}
async function listDeployments(request, env) {
    const url = new URL(request.url);
...
async function getDeployment(env, id) {
    const row = await env.DB.prepare('SELECT * FROM deployment_jobs WHERE id = ?').bind(id).first();
    if (!row)
        return json({ message: 'Deployment not found' }, { status: 404 });
    const logs = await env.DB.prepare('SELECT * FROM deployment_logs WHERE deployment_id = ? ORDER BY created_at ASC').bind(id).all();
    return json({
        id: row.id,
...
async function adminListTemplates(request, env) {
    requireOwnerSession(request, env);
    const result = await env.DB.prepare('SELECT * FROM templates WHERE deleted_at IS NULL ORDER BY updated_at DESC').all();
    return json(await Promise.all(result.results.map(row => rowToTemplate(env, request, row))));
}
async function adminUpdateTemplateMeta(request, env, slug) {
    requireOwnerSession(request, env);
    const body = await readJson(request);
    const row = await env.DB.prepare('SELECT * FROM templates WHERE slug = ? AND deleted_at IS NULL').bind(slug).first();
    if (!row)
        return json({ message: 'Template not found' }, { status: 404 });
    await env.DB.prepare('UPDATE templates SET label_json = ?, description_json = ?, icon = ?, preview_image = ?, highlights_json = ?, theme_names_json = ?, updated_at = ? WHERE slug = ?').bind(JSON.stringify(body.label || parseJson(row.label_json, { fr: slug, en: slug })), JSON.stringify(body.description || parseJson(row.description_json, { fr: '', en: '' })), body.icon || row.icon, body.previewImage ?? row.preview_image, JSON.stringify(Array.isArray(body.highlights) ? body.highlights : parseJson(row.highlights_json, [])), JSON.stringify(Array.isArray(body.themeNames) ? body.themeNames : parseJson(row.theme_names_json, [])), nowIso(), slug).run();
    const updated = await env.DB.prepare('SELECT * FROM templates WHERE slug = ?').bind(slug).first();
    return json(await rowToTemplate(env, request, updated));
}
async function adminListReleases(request, env) {
    requireOwnerSession(request, env);
    const result = await env.DB.prepare('SELECT * FROM releases ORDER BY created_at DESC').all();
    return json(result.results.map(row => decorateRelease(request, env, row)));
}
async function adminUpdateReleaseMeta(request, env, version) {
    requireOwnerSession(request, env);
    const body = await readJson(request);
    const row = await env.DB.prepare('SELECT * FROM releases WHERE version = ?').bind(version).first();
    if (!row)
    ...
    const updated = await env.DB.prepare('SELECT * FROM releases WHERE version = ?').bind(version).first();
